...
class TimeNormalizer:
    ...
    def init(self):
        ...
        try:
            ...
            with io.open(os.path.dirname(__file__) + '/resource/regex.txt', 'r', encoding='utf-8') as f:
                content = f.read()
            ...
            with open(fpath, 'rb') as f:
                pattern = pickle.load(f)
        with io.open(os.path.dirname(__file__) + '/resource/holi_solar.json', 'r', encoding='utf-8') as f:
            holi_solar = json.load(f)
        with io.open(os.path.dirname(__file__) + '/resource/holi_lunar.json', 'r', encoding='utf-8') as f:
    ...
    def parse(self, target, timeBase=arrow.now()):
        ...
        self.timeBase=self.timeBase.encode('utf-8')
    ...
    def __timeEx(self):
        ...
        rpointer = 0
        temp = []
        match = self.pattern.finditer(str(self.target))

        r = [ a.group() for a in list(match)]
        print('\xe6\x9c\x88'.encode('utf-8'))
